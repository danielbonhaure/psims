...
class DSSATXFileOutput:
    ...
    def __init__(self, exp_file, soil_file, cul_file, use_ptransfer = True): # need soil file for initial soil conditions
        ...
        for i in range(len(self.exps)):
            ...
            soil_profile = [soil_id, delta_cly]
            element_found = False
...
class SOLFileOutput:  
    ...
    def __init__(self, soil_file, exp_file, use_ptransfer = True): # need experiment file to know which soil profiles to write 
        ...
        for i in range(len(exps)):
            if 'soil_id' in exps[i]:
                soil_id = exps[i]['soil_id']
                if len(soil_id) > 10 and soil_id.find('_') != -1:
                    idx = soil_id.find('_')
                    soil_id = soil_id[: idx]
            else:
                soil_id = 'XY01234567'
            if 'delta_cly' in exps[i]:
                delta_cly = exps[i]['delta_cly']
            else:
                delta_cly = '0'
            soil_idx = soil_ids.index(soil_id)
            soil_profile = {'soil_id': soil_id, 'delta_cly': delta_cly, 'soil_idx': soil_idx}
            element_found = False
            for j in range(len(self.soil_profiles)):
                if self.soil_profiles[j] == soil_profile: element_found = True
            if not element_found: self.soil_profiles.append(soil_profile)
...
parser.add_option("-c", "--CULfile", dest = "CULfile", default = "MZCER045.CUL", type = "string",
                  help = "output CUL file", metavar = "FILE")
parser.add_option("--pfcn", action = "store_true", dest = "pfcn", default = False,
                  help = "Whether to use pedotransfer functions (default = False)")
options, args = parser.parse_args()

# parse experiment JSON file and write out X file
xfileoutput = DSSATXFileOutput(options.efile, options.sfile, options.CULfile, use_ptransfer = options.pfcn)
xstr, cstr = xfileoutput.toXFile()
open(options.Xfile, 'w').write(xstr)
if cstr != '': open(options.CULfile, 'w').write(cstr)
