...
parser.add_option("--collection_ids", dest="collection_ids", default=None, type="string",
                  help="MongoDB collection id's file for each latidx and lonidx point in a JSON array")
...
id_files = options.collection_ids
collection_id = None

if id_files:
    collection_file = json.load(open(id_files))
    if 'ids' not in collection_file:
        raise Exception("Collection ids should be inside the 'ids' field in the JSON file.")

    if 'oids' not in collection_file:
        collection_file['oids'] = 'false'

    collection_id = collection_file['ids'][latidx][lonidx]
    if collection_file['oids'] in ['True', 'true', 'yes', True]:
...
for var_idx, var_name in enumerate(variables):
    ...
    elif var_name in var_units:
        var_units = var_units[var_name]

    var_fullname = var_name
    if var_name in var_names:
        var_fullname = var_names[var_name]

    mongo_object[var_name] = {
        'name': var_fullname,
        'units': var_units,
        'scenarios': []
    }
    results = mongo_object[var_name]['scenarios']
    var_content = trim_data[: num_years * num_scenarios, var_idx].tolist()

    for scen_idx in range(0, num_scenarios):
        s_content = var_content[scen_idx*num_years:(scen_idx+1)*num_years]
        s_name = str(scen_idx+1)
        if scen_names:
            s_name = str(scen_names[scen_idx])
...
if collection_id:
    ...
    results = db[mongo_collection].update_one(doc_query, update_query, upsert=True)
else:
    results = db[mongo_collection].insert_one(mongo_object)
