...
with nc(ifile) as f:
    ...
    if yieldvar:
        yieldv = f.variables[yieldvar][slicer] # pull yield
...
if weightsf:
    with nc(weightsf) as f:
        ...
        rfweights = f.variables['rainfed'][sellat][:, sellon]
        irweights = f.variables['irrigated'][sellat][:, sellon]
...
createAggFile(outputf, time, tunits, audata, anames, aunits, alongnames, scensel, irr, leaddim)
f = nc(outputf, 'a')
